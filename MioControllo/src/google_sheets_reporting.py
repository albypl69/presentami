        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                if os.path.exists(CREDENTIALS_PATH):
                    ...
                    print(f'The Google Sheets credential file `{CREDENTIALS_PATH}` does not exist or is not in the proper path. Cannot write to Google Sheets. Please refer to the README section `Google Sheets API Instructions (Optional)` for further instruction.')
                    return
